/**
   ...
   @date Last modified: 2019-05-07T09:49:49-0400
...
class ConfigMem {
  public:
    ...
    static uint8_t MaxPatternSet; //!< Max number of pattern sets in EEPROM
    ...
    static uint16_t RandomBase;   //!< Base address of Pattern Sets in EEPROM
};
...
class Flash {
  public:
    ...
    uint16_t OnDuration = 0;         //!< Time at full illumination
...
/**
   ...
   @brief   Define a `random pattern set` by a list of patterns that will be
            randomly selected and executed

            A pattern is "selected" by setting a corresponding bit to '1' in
            the PatternSet variable. Setting bit 0 to '1' adds pattern number
            1 to the random set, setting bit 1 adds pattern number 2, and so
            on. Note that PatternSet is defined as a 16-bit integer.

   @todo    Verify that datatype of PatternSet has MaxPattern bits,
            preferably at compile time
*/
class RandPatternSet {
  public:
    uint8_t Number = 0;        //!< From 1 to MaxPatternSet
    uint16_t PatternSet;       //!< Bitfield to enable patterns

    bool Save(void);
    void Get(uint8_t setnum);
    static bool isDefined(uint8_t setnum);
    void Display();
};
